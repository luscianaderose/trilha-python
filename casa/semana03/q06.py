...
# Continuando o exercício anterior, onde você implementou a funcionalidade de remover 
# produtos do estoque, agora o sistema deve permitir que o usuário atualize as
# informações de um produto já existente no estoque.

# Tarefas:

# 1. Atualizar Produto:
# O sistema deve permitir que o usuário atualize as informações de um produto. 
# O usuário deverá escolher um produto pelo seu identificador e fornecer as
...
# Após atualizar as informações do produto, o sistema deve exibir a lista atualizada de produtos, 
...
id_alterar = int(input('\nDigite o identificador do produto que deseja alterar: ')) - 1
nome_original = list(produtos.keys())[id_alterar]
novo_nome = input('Digite o novo nome (ou pressione Enter para manter o nome atual): ') or nome_original
novo_preco = input('Digite o novo preço (ou pressione Enter para manter o preço atual): ').strip()
nova_qtd = input('Digite a nova quantidade (ou pressione Enter para manter a quantidade atual): ').strip()

if novo_nome != nome_original:
    produtos[novo_nome] = produtos.pop(nome_original)

if novo_preco:
    produtos[novo_nome]['preço'] = float(novo_preco)

if nova_qtd:
    produtos[novo_nome]['quantidade'] = int(nova_qtd)

print(f'Produto {novo_nome.capitalize()} alterado com sucesso.')
